...
| **role 主干度** | 纵向·外部·补集角色（不动点） | 2026-08-14 实验 | 偏序图角色迭代（含 U2 退化）；**救援门**救回被熵门误杀的 000 层真词 | `--role-rescue 0.7` |
| **asym 不对称性** | 纵向·外部·条件熵差 | 2026-08-14 实验 | 复合熵的逆：被虚词修饰的真词特征；**救援门** | `--asym-rescue 2.0` |
...
`role_rescue=0.7`、`asym_rescue=2.0`——两者都做**救援门**（从被 AND 链滤掉的词里捞回），
**绝不做过滤门**（实测 role/asym 过滤门都是灾难：keep 36→14 崩盘、000 零回收）。

**效果**（68 档全量交并实测，`调参工具/run_full_union.py`）：

| 指标 | base（无 role/asym） | +role0.7+asym2.0（默认） |
|---|---:|---:|
| 词数 | 5149 | 5413 |
| 000 层真词 | 0/15 | **13/15** |
