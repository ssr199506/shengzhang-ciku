...
## 四·五、项目精髓：从抽象思考到 role/asym

> 完整过程见 `docs/生长词库_完整生命周期演进文档.md` 第六、十一章。这里摘录三段最能代表项目方法论思考的演进。

### 1. 抽象思考直接推导出「凝固度 × 偏序」组合

本项目最独特的资产是把每个算法抽象成**维度形态**，而非公式数值：

| 信号 | 维度形态 |
|---|---|
| 复合熵 | 横向·外部——邻居位置上能不能换人 |
| 位置固定度 | 横向·边界特例——是否焊死在边界位（补丁） |
| 凝固度 | 纵向·内部——内部两半是绑定还是偶遇 |
| SPE / 偏序 | 纵向·外部——在更长超词里的构词位/角色 |

**「凝固度 × 偏序」的组合是纯抽象直觉得出的，不是调参调出来的**：凝固度看词的**内部**（字间结合紧密度），偏序看词的**外部角色**（在超词里是主干还是附件），两个维度正交；且二者共用同一张 `freq` 表——每个子串既是"凝固度的一块拼图"，又是"偏序结构的一个节点"，**零额外统计成本**。这一判断经实验证实（2.3.3 indep 联合门）。

### 2. 抽象思考引导出 role / asym 演进

三条失败路线（独立率陷阱 / SPE 撞墙 / RSR 陷阱）被统一诊断为**同源：测错变量**——都在用便宜的代理变量替代真正的结构变量。由此推出突破口：

> 虚词在偏序图中几乎总是**附件**（只修饰、从不被当作主体）；实词可以"被叠加"（从+军=从军，军是主体）。

虚词身份不由字典规定，而由**偏序图不动点迭代**自己涌现（HITS 式 hub/authority）——这就是 **role 主干度**；再配以 **asym 不对称性**（`H(w|r)−H(r|w)`，复合熵的逆）测量"谁依附谁"。二者均落地为救援门，成为当前 main 的默认信号。

### 3. 过滤门 vs 救援门不可兼得（猜想 → 实验部分证实）

**猜想**：对每个新信号，若"基线保留真词"与"被滤碎片"的信号分布重叠，则过滤门与救援门**不可兼得**——过滤必砍真词，救援必混碎片。

**实验证实**（全量交并实测）：分离带判据 `max(应滤∩G) < min(真词∩B)` 成立才可兼得；实测 role 分离带 −0.7285、asym 分离带 −7.7445，**双双混叠**——过滤门必伤真词（keep 36→14 崩盘），救援门必带碎片（碎片 +3）。这解释了为什么 role/asym **只能做救援门、绝不能做过滤门**，也是"救援门带噪声是结构性代价、非调参可救"的数学根源。
